...
from ac_o import AntColonyOptimizer
weather_data = {}
marine_data = {}
...
async def handle_navigation(websocket):
    try:
        ...
        G = load_navigation_graph(
            "D:/Semister 8/Major Project/Project/Optimal Ship Routing using AI/Graph/grid_based_ship_routes.graphml"
        )
        ...
        smooth_astarpath = bspline_smooth(a_star_path, smoothing_factor=0)
        plot_subgraph(subgraph, smooth_astarpath)
        ...
        optimized_path = AntColonyOptimizer(optimized_subgraph, start_node, end_node, weight='weight')
        
        print(optimized_path)

        smooth_path = bspline_smooth(optimized_path, smoothing_factor=0.2)
        
        # Plot optimized path
        plot_subgraph(optimized_subgraph, smooth_path)
